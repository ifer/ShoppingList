...
function getFormattedDate(){
    ...
    var ds = ('0' + d.getDate()).slice(-2)  + "/" + ('0' + (d.getMonth() + 1)).slice(-2) + "/"+  d.getFullYear() + " " + ('0' + d.getHours()).slice(-2) + ":" + ('0' + d.getMinutes()).slice(-2) + ":" + ('0' + d.getSeconds()).slice(-2);
...
function isUserAdmin (curruser){
	...
	for (let i=0; i<curruser.roles.length; i++){
		if (curruser.roles[i] === 'ROLE_ADMIN')
			return true;
	}
	return (false);
}
...
function isUserReadonly (curruser){
	...
	for (let i=0; i<curruser.roles.length; i++){
		if (curruser.roles[i] === 'ROLE_READONLY')
			return true;
	}
	return (false);
}
...
export {datestrToIsoDate, noGreekAccents, getFormattedDate, getContentHeight, isUserAdmin, isUserReadonly, dateValidationState, calcAge, callAppTrace};
