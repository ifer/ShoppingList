...
//            	onError();
	...
	openDelCategoryForm (){
		...
		let customBody = (
				<div>
			      <div>
			        {messages.deleteCategoryConfirmBody1}
			      </div>
			       <span className='text-primary'>{curCategory.descr}</span>
			       <div><p> </p> </div>
			       <div>
			        {messages.deleteCategoryConfirmBody2}
			      </div>
			    </div>
			    );
	          ...
	          <Row>
	            <Col md={4}>
		          	<ButtonGroup bsClass="expenses-button-group">
						<Button bsStyle="success" onClick={this.openAddCategoryForm} className="table-action-button">{messages.action_add}</Button>
						<Button bsStyle="info" onClick={this.openUpdCategoryForm} className="table-action-button">{messages.action_update}</Button>
						<Button bsStyle="danger" onClick={this.openDelCategoryForm}  className="table-action-button">{messages.action_delete}</Button>
					</ButtonGroup>
				</Col>
				</Row>
			</Grid>
			<div className="categories-table">
		     <BootstrapTable data={ this.state.categories } options={ options }  striped hover condensed height={this.state.tableheight}  ref="CategoryTable" selectRow={ this.selectRowProp } >
		          <TableHeaderColumn dataField='catid' defaultValue='0' hidden isKey>{messages.CategoryId}</TableHeaderColumn>
		          <TableHeaderColumn dataField='descr' width="50%" >{messages.categoryDescr}</TableHeaderColumn>
	          </BootstrapTable>
	        </div>
			<CategoryForm ref='CategoryForm' onModify={this.updateCategory} sharemethods={this.state.sharemethods}/>
			<Dialog ref='dialog' />
