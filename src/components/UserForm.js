...
//		}
		...
		if (this.passwords.passwd1 != null)
			this.userobj.passwd = this.passwords.passwd1;
		else
			this.userobj.passwd = null;
...
//	    console.log (name +": " + this.userobj[name]);
	...
	getFormValidationState (){
		...
		if (this.getNameValidationState () == 'error')
			return 'error';
...
//		}
		...
		if (this.getRolesValidationState() == 'error')
			return ('error');
...
//		}
	...
	getNameValidationState (){
		if (this.userobj.name == null || this.userobj.name == '')
			return ('error');
	}

	getPasswordValidationState (checkStrength){
		if (this.isNewUser){
			if (this.passwords.passwd1 == null || this.passwords.passwd2 == null)
				return ('error');
		}
	...
	checkPasswordStrength () {
		...
		if (this.passwords.passwd1 == null) //Not initialized yet
			return ('error');
	...
	getRolesValidationState (){
		...
		if (this.userobj.roles == null || this.userobj.roles == '') // Cannot be empty
			return ('error');
		...
		for (let i=0; i<valid_occurrences.length; i++)
			valid_occurrences[i] = 0;


		for (let j=0; j<roles.length; j++){                 //Match actual roles with valid ones and increase valid counters
			for (let i=0; i<valid_roles.length; i++){
				if (roles[j].trim() === valid_roles[i].trim())
					valid_occurrences[i]++;
				if (valid_occurrences[i] > 1)               // A role cannot appear more than once
					return ('error');
			}
		}

		let sum=0;
		for (let i=0; i<valid_occurrences.length; i++){                     // Sum valid occurrences
			sum += valid_occurrences[i];
		}

		if (sum == roles.length)                            // All actual roles must be valid
			return ('success');
	      ...
	      <div>
	        <Modal show={this.state.showUserForm} onHide={this.cancel} keyboard={true}  dialogClassName="">
	          ...
	          <Modal.Body>
	           ...
	           <Col sm={8}>
	             	<FormControl name="name" readOnly={! this.props.isAdmin} type="text" defaultValue={this.userobj.name} onChange={this.handleInputChange} />
	           </Col>
	          </FormGroup>
	           ...
	           <FormGroup controlId="passwd1" validationState={this.getPasswordValidationState(true)} >
	            ...
	            <Col sm={8}>
	              	<FormControl  name="passwd1" type="password" defaultValue={''} onChange={this.handleInputPassword} />
	            </Col>
	           </FormGroup>
	           ...
	           <FormGroup controlId="passwd2" validationState={this.getPasswordValidationState(true)} >
	            ...
	            <Col sm={8}>
	              	<FormControl  name="passwd2" type="password" defaultValue={''} onChange={this.handleInputPassword} />
	            </Col>
	           </FormGroup>

	          <FormGroup controlId="roles"  onChange={this.handleInputChange}  validationState={this.getRolesValidationState()}>
        		<Col componentClass={ControlLabel} sm={4}>
        			{messages.usersRoles}
        		</Col>
	            <Col sm={8}>
	              	<FormControl  name="roles" type="text" readOnly={! this.props.isAdmin} defaultValue={this.userobj.roles} onChange={this.handleInputChange} />
